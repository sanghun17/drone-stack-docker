...
# Pass a per-app display/port to isolate a GUI, or the SAME triplet to several so they share ONE
# browser tab (the rviz/rqt/realsense/diagnostic wrappers do the latter). Independence is per-PROCESS — start/
# stop one app without the rest — NOT per-display. The
...
docker exec -i "$C" env DN="$DN" P="$P" WP="$WP" PROC="$APPNAME" MATCH="${MATCH:-}" MATCH_NOT="${MATCH_NOT:-}" W="$W" bash -s -- "$@" <<'EOSH'
D=":$DN"
...
# app window present? lines matching MATCH (default = PROC), minus an optional MATCH_NOT regex. The
# include/exclude pair tells apart apps that share a WM_CLASS *and* Qt's aux windows (plain rqt vs
# rqt_runtime_monitor — both class "rqt", both spawn a "Qt Selection Owner for rqt") on a SHARED
# display. Window-grep, not pgrep (zombies fool pgrep — see header).
app_up(){ local t; t=$(xwininfo -display "$D" -root -tree 2>/dev/null | grep -iE "${MATCH:-$PROC}")
  [ -n "$MATCH_NOT" ] && t=$(printf '%s\n' "$t" | grep -viE "$MATCH_NOT"); [ -n "$t" ]; }
...
if ! app_up; then
  ...
  for i in $(seq 1 20); do app_up && break; sleep 1; done
fi
EOSH
