...
for that stack. Driven by setup.sh.
    ...
    python3 scripts/gen_dockerfile_compose.py <stack> [--arch arm64|amd64] [--gpu-arch sm120]
...
def gen_dockerfile(mods, arch, gpu_arch, env):
    ...
    L.append("# GENERATED by scripts/gen_dockerfile_compose.py — do not edit. Stack arch=%s%s"
...
def gen_compose(mods, arch, stack, env, gpu_uuids_key="GPU_UUIDS", gpu=True,
    ...
    #
    # gpu_uuids_key (optional, stacks/*/stack.yml `gpu_uuids_env:`, added 2026-07-25 for
    ...
    header = "# GENERATED by scripts/gen_dockerfile_compose.py. run scripts for this stack:\n#   " + \
             "\n#   ".join(runs) + "\n"
...
def main():
    ...
    sf = os.path.join(ROOT, "stacks", a.stack, "stack.yml")
    ...
    # `gpu:` (optional, stacks/*/stack.yml) — set false for a stack that must run on a host
    # with no NVIDIA GPU. Defaults true, so every pre-existing stack is unaffected.
    gpu = stack.get("gpu", True)
    # `ros_master_port:` (optional, stacks/*/stack.yml) — pin this stack's container to its
