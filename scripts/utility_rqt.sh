...
# judged "already up" when the monitor is open and never starts. (Infra logic in scripts/lib/vnc_gui.sh.)
exec env MATCH='"rqt" "rqt"' MATCH_NOT="rqt_runtime_monitor" "$(dirname "$(readlink -f "$0")")/lib/vnc_gui.sh" 99 5900 6080 rqt rqt "$@"
