...
> # ✅ 최종 결론 (10라운드 `53264b3`) — 배포 후보 확정, 위험은 수치로
>
> **`SW_r27`을 seed 6개로 돌린 결과, 9라운드의 "2/3"는 n=3의 착시였다.** s7은 **6개 중
> 유일한 이탈**이고 나머지 5개는 촘촘하다:
>
> | seed | holdout(best) | unc% |
> |---|---|---|
> | s1337 / s777 / s42 / s2024 / s123 | 0.589 / 0.674 / 0.676 / 0.703 / 0.800 | 29.2~31.4 |
> | **s7** | **1.566** | **40.9** |
>
> s7만 **두 축이 함께** 무너진다. s123은 홀드아웃이 약간 높지만(0.800) uncertainty는
> 31.4%로 정상군과 구분이 안 된다 — 이탈이 아니라 정상 범위의 위쪽 끝이다.
> IQR 상단 울타리 0.987을 넘는 것도 s7뿐. s7 제외 시 평균 0.688 / 표준편차 0.075.
>
> **배포 판단: `SW_r27` 유지 — 단 "검증된 승자"가 아니라 위험을 수치화한 추천.**
> 관측 이탈률 **1/6(~17%)**, 다만 n=6의 95% 신뢰구간이 **약 3~56%**로 넓다.
>
> **실무 지침**: 배포할 **특정 체크포인트를 홀드아웃으로 직접 검증한 뒤** 출하할 것.
> 새로 학습한 인스턴스를 맹신하지 말고, **2~3 seed를 돌려 홀드아웃으로 고르는 것**이
> 이 캠페인이 실제로 관측한 나쁜 draw를 피하는 값싼 보험이다.
>
> ⚠ **in-domain으로 seed를 고르면 안 된다** — 6 seed의 best_val이 3.4182~3.4717(폭 0.0535)로
> 촘촘한데 홀드아웃은 2.3배 이상 갈린다.
>
> *(아래 9라운드 블록은 n=3 시점의 판단이다. 방향은 맞았지만 — seed가 결정적이다 —
> "어떤 arm도 추천할 수 없다"는 강도는 n=6에서 완화됐다. 이력으로 남긴다.)*
>
> ---
>
> # 🛑 9라운드 시점 판단 (`3a90846`, n=3)
>
> **9라운드·18 arm 을 돌렸지만, seed 를 실제로 n=3으로 검증하자 어떤 arm 도 "안정적으로
> 이긴다"고 말할 수 없다.** 이 홀드아웃에서 **seed 간 분산이 9라운드 내내 재온 config 차이
> 대부분과 같거나 더 크다.**
>
> `SW_r27`(8라운드까지의 추천)을 n=3으로 스트레스 테스트하려고 seed 7을 돌렸더니 **갈렸다**:
>
> | 에폭 | s42 | **s7** | s1337 | | s42 unc% | **s7 unc%** | s1337 unc% |
> |---|---|---|---|---|---|---|---|
> | 100 | 0.6518 | **1.6200** | 0.5826 | | 29.5 | **41.9** | 29.2 |
> | best | 0.6757 | **1.5660** | 0.5893 | | 30.0 | **40.9** | 29.2 |
>
> s7이 전 지점에서 **2.2~2.5배 나쁘고** uncertainty도 ~40%로 **구 배포모델(37.6)보다 나쁘다**.
> 직전 라운드에서 `DNoff`를 무너뜨린 것과 같은 크기·같은 방향의 실패다. **`SW_r27`은
> 3/3이 아니라 2/3다.**
>
> ⚠ **in-domain은 이 발산을 전혀 예고하지 못했다** — 세 seed의 best_val이 3.4562 / 3.4688 /
> 3.4717로 **촘촘히 모이고**(폭 0.0155) 정점 에폭도 ep115~118로 같은데, 홀드아웃은 2.5배
> 갈린다. 이 캠페인이 반복해서 만난 in-domain/실기 괴리의 가장 극단적인 형태다.
>
> ### 지금 결정해야 한다면
>
> `SW_r27`이 여전히 단일 arm 중 근거가 가장 강하다(3 seed 중 다수, 유일한 경쟁자 `DNoff`는
> 서사가 완전히 뒤집힘). 다만 **"검증된 승자"가 아니라 "알려진 큰 seed 위험 하의 최선의
> 베팅"**으로 다루고, **레시피/seed 조합을 믿지 말고 실제 체크포인트를 이 홀드아웃(또는 새
> 홀드아웃)으로 직접 검증한 뒤** 출하할 것.
>
> **다음 최우선은 새 아키텍처 arm이 아니라, 선택한 레시피를 여러 seed로 돌려 보는 것이다.**
> seed 정체성이 설계 선택만큼 중요하다는 게 이제 두 번 실증됐다.
>
> ---
>
> # 🛑 §2.1 헤드라인 정정 (`3c72ded`, 8라운드)
>
> 아래 §1·§2.1은 **"DN-const 해제(`dirichlet_dn_fixed_log_n: null`)가 uncertainty 폭발의
> 답"**이라고 쓰고 있다. 그 근거는 `DNoff_nosw` **seed 42 하나**였고, **공정한 재현에
> 실패했다.**
>
> `min_delta`를 s42가 돌던 1e-4로 맞춘 대조군(`DNoff_md1e4_s1337`, ep393까지 도달)에서:
>
> | | holdout | unc%자연 |
> |---|---|---|
> | 구 배포 `deploy1_s42_ref` | 1.2174 | 37.6 |
> | `DNoff_nosw` (s42) | 0.7444 | **26.1** |
> | `DNoff_md1e4_s1337` | **1.9370** | **39.0** ← 구 모델보다 나쁨 |
> | `SW_r27` (s42) | 0.6757 | 30.5 |
> | `SW_r27_s1337` | **0.5893** | **29.2** ← seed 안정 |
>
> **같은 config·같은 min_delta·같은 예산, seed만 다른데 후반부가 3배 벌어진다**(ep300:
> 0.666 vs 2.067). DNoff의 "에폭이 늘수록 좋아진다"도, uncertainty 개선도 **재현되지 않는
> seed-42 아티팩트**다.
>
> **바뀌지 않는 것**: 최종 배포 후보는 여전히 **`SW_r27`**이다. 오히려 대조가 선명해졌다 —
> 두 seed 모두 구 모델(37.6)보다 uncertainty가 낫고(30.5 / 29.2) 홀드아웃도 s1337이 더
> 좋은, **캠페인에서 유일하게 seed를 넘어 재현된 arm**이다.
>
> **바뀌는 것**: `DNoff_nosw`를 2순위 후보에서 **강등**한다. s42 수치가 틀린 건 아니지만,
> 그 수치를 설명하던 메커니즘이 공정한 재현에서 실패했으므로 **설계상의 승리로 제시할 수
> 없다.** §2.1을 이 관점으로 읽을 것.
