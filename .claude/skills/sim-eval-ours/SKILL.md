...
`modules/planner/risk-aware/run_*_sim.sh and stacks/sim-x86/scripts/run_*.sh` (+ FAST-LIVO는 `modules/odometry/fast-livo/run.sh`)를
...
- 센서 퍼블리셔는 `LOC=vio`로 다시 (기본 gt 대신):
  ```bash
  LOC=vio bash /home/ml/drone-stack-docker/stacks/sim-x86/scripts/run_sensor_pub.sh
  ```
```bash
...
bash /home/ml/drone-stack-docker/stacks/sim-x86/scripts/run_so3.sh
bash /home/ml/drone-stack-docker/stacks/sim-x86/scripts/run_eval.sh     # algorithm:=ours 스크립트에 이미 하드코딩
```
