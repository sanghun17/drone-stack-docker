#!/usr/bin/env python3
# Parallel NO-ALIGN position-RMSE eval of FAST-LIVO2 tuning replays. Reads _t5o_*.bag directly via
...
def main():
    ...
    bags = sorted(glob.glob(os.path.join(d, "_t5o_*.bag")))
    with Pool() as p:
        res = p.map(ev, bags)
    ...
    o = ["FAST-LIVO2 tune5 — NO-ALIGN position RMSE [m] of /aft_mapped_to_optitrack vs pure (time-corrected)",
         "handles: {imu}_v{img}_b{bias}_l{lto}_d{dep}   (%d/%d evaluated)" % (len(ok), len(bags)), "",
    ...
    for lab, pos, vals in [("IMU_trust", 0, ["lo", "mid", "hi"]), ("img_point_cov", 1, ["v100", "v1000"]),
                           ("bias_RW", 2, ["blo", "bmid", "bhi"]), ("lidar_off", 3, ["l000", "l005", "l010"]),
                           ("dept_err_rel", 4, ["d01", "d02", "d03"])]:
