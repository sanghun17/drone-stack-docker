#!/bin/bash
# control/local-controller: standalone standard-message trajectory controller.
# trajectory_msgs/MultiDOFJointTrajectory -> geometric PD velocity controller ->
# flight_safety MUX -> MAVROS/PX4. Commands are DISABLED by
...
if [ ! -f /.dockerenv ]; then
  ...
  dsd_select_stack "control/local-controller" || exit $?
  : "${DSD_CONTAINER:?set DSD_CONTAINER or invoke through './setup.sh run <stack> control/local-controller'}"
  __C="$DSD_CONTAINER"
...
exec taskset -c "${CPUS_POOL:?config/ros_env.sh not sourced}" \
  python3 /work/modules/control/local-controller/local_controller_node.py "$@"
