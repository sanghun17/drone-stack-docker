# `libraries/torch/wheels/` — source-build torch (ABI=1), the sm89/sm75 default

This directory carries `torch-2.2.2-cp38-cp38-linux_x86_64.whl`, the source-built
PyTorch installed by `modules/libraries/torch/install.sh`'s `sm89|sm75` branch —
...
`stacks/sim-x86/stack.yml`) — see modules/training/ete-net/DEPLOYMENT.md's "torch unification"
...
See `modules/libraries/torch/install.sh`'s `sm89|sm75` branch for the install-time
...
limit — unlike `modules/libraries/jax/wheels/*.whl`, which IS small enough to track).
This README.md IS git-tracked so the directory survives a fresh clone — if it were
missing, the BuildKit `RUN --mount=type=bind,source=modules/libraries/torch/wheels,...`
...
for ALL stacks that include `libraries/torch`, not just sim-x86.
...
`modules/training/ete-net/DEPLOYMENT.md`'s "torch unification" section for the file:line change
