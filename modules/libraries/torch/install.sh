#!/bin/bash
# libraries/torch: PyTorch 2.1.0. Ported from pure-jetson-stack Dockerfile `torch` stage.
...
case "${TARGETARCH:-arm64}" in
  arm64)
    ...
    TORCH_URL="https://developer.download.nvidia.com/libraries/redist/jp/v512/pytorch"
  ...
  amd64)
    ...
    case "${GPU_ARCH:-}" in
      sm89|sm75)
        ...
        WHL=$(ls /modules/libraries/torch/wheels/torch-2.2.2-cp38-cp38-linux_x86_64.whl 2>/dev/null | head -1)
        [ -n "$WHL" ] || { echo "ERROR: modules/libraries/torch/wheels/ 에 torch 휠이 없다 -- modules/libraries/torch/wheels/stage_from_archive.sh 를 먼저 실행해라 (TORCH_WHEEL_ARCHIVE_DIR 필요, git 미추적 274MB — README.md 참조)." >&2; exit 1; }
        ...
        # torch 자신을 위한 게 아니라, 같은 이미지에 libraries/jax가 함께 들어가는 스택(sim-x86)의
        # jaxlib(0.4.13+cuda12.cudnn89, 시스템 libcudnn.so.8 dlopen 요구)를 위한 것 -- libraries/jax가
