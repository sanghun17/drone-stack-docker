#!/bin/bash
# libraries/jax: jaxlib 0.4.13 (baked wheel default, else source) + jax + flax/optax.
# Ported from pure-jetson-stack Dockerfile `jax` stage. No aarch64 pip wheel for 0.4.13.
set -e

case "${TARGETARCH:-arm64}" in
arm64)
WHEELS=/modules/libraries/jax/wheels
...
amd64)
  ...
  # python3.8, no python-version shim needed -- same reasoning as libraries/torch's
  # sm75/sm89 branch):
