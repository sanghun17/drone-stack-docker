...
combo-key lookups — see `modules/SCHEMA.md`):

| stack | host GPU | driver constraint | CUDA container line | torch |
|---|---|---|---|---|
| `stacks/ete-train-5090/stack.yml` | RTX 5090 (Blackwell, sm_120) | none known | `nvidia/cuda:12.8.1-devel-ubuntu20.04` | official pip cu128 wheel (unaffected by the sm89/sm75 unification below) |
| `stacks/ete-train-4090/stack.yml` | RTX 4090 (Ada, sm_89) | driver 535.183 -> CUDA<=12.2 containers only | `nvidia/cuda:12.2.2-devel-ubuntu20.04` | source-built torch 2.2.2 (ABI=1, CUDA-unbundled) — since 2026-07-26, same wheel `sim-x86` uses (see "torch unification" section) |
| `stacks/ete-train-2080ti/stack.yml` | RTX 2080 Ti x3 (Turing, sm_75) -- the ML desktop itself | driver 535.261 -> CUDA<=12.2 containers only (same ceiling as the 4090 host) | `nvidia/cuda:12.2.2-devel-ubuntu20.04` (reuses `amd64_sm89`'s value) | source-built torch 2.2.2 (ABI=1, CUDA-unbundled) — since 2026-07-26, same wheel `sim-x86` uses (see "torch unification" section) |
...
Modules (same for both stacks): `base` (always-on) + `libraries/torch` +
`libraries/spconv` + `training/ete-net`.
...
- **What actually works**: the OLDER `--runtime=nvidia` + `NVIDIA_VISIBLE_DEVICES=<uuid,...>`
  ...
  `scripts/gen_dockerfile_compose.py` via the `GPU_UUIDS` env var (`config/stack.env`) --
...
GPU_UUIDS=<healthy-uuid1>,<healthy-uuid2>,... CONTAINER_USER=$(id -u):$(id -g) \
  python3 scripts/gen_dockerfile_compose.py ete-train-2080ti --arch amd64
...
- **sm_120 (5090)**: if this fails with a kernel-image error,
  ...
  fallback in `modules/libraries/spconv/install.sh` needs its `CUMM_CUDA_ARCH_LIST`
...
axis (`scripts/gen_dockerfile_compose.py`) that exists ONLY to scope the variant to one
...
- **`modules/libraries/torch/install.sh`** -- the `sm89|sm75)` case (lines 20-105) wraps
...
- **`stacks/sim-x86/stack.yml`** lines 19-25 (`build_env: {TORCH_VARIANT: src-abi1}` +
  comment) become dead weight once src-abi1 is unconditional -- delete.
- **`scripts/gen_dockerfile_compose.py`** -- the `build_env` plumbing itself (module
...
- **`modules/libraries/torch/wheels/README.md`** and **`modules/libraries/torch/module.yml`**
  description -- both currently describe the conditional (`UNLESS the stack sets
  build_env...`); rewrite once unconditional.
- **`modules/training/ete-net/DEPLOYMENT.md`** -- the per-stack table's "CUDA container line" /
...
- **NOT required to change**: `stacks/ete-train-4090/stack.yml` / `ete-train-2080ti.yml` /
...
Reused sim-x86's existing pattern (`modules/libraries/torch/wheels/` + `assets: [wheels]`
...
- **New script**: `modules/libraries/torch/wheels/stage_from_archive.sh` -- reads
  `TORCH_WHEEL_ARCHIVE_DIR`, hardlinks (falls back to copy across filesystems) the
  wheel into `modules/libraries/torch/wheels/`, md5-verifies, idempotent. Run once per
...
3. **CPU qr**: `torch.linalg.qr(torch.randn(4,4))` runs on CPU -- **PASS** (this is the
   ...
   `modules/libraries/torch/wheels/README.md`).
...
warnings):
1. Apply the `install.sh`/`stacks/sim-x86/stack.yml`/generator cleanup from §1.
2. Run `modules/libraries/torch/wheels/stage_from_archive.sh` on that host (already done
...
unconditional for the `sm89|sm75` case; `stacks/sim-x86/stack.yml`'s `build_env:` block
removed; `scripts/gen_dockerfile_compose.py`'s `build_env` plumbing removed entirely --
...
`stacks/ete-train-4090/stack.yml`/`ete-train-2080ti.yml` comments updated to say
...
`libraries/jax` (sim-x86), not torch itself).
...
`libraries/torch`'s) -- no other line changed. This confirms the cleanup is
...
- `modules/libraries/torch/module.yml`, `install.sh` — amd64 branch: python3.11 shim (deadsnakes; cu128 wheels need >=3.9, base image ships 3.8) + official PyTorch cu128 wheels. **UNVERIFIED end-to-end on real hardware** (apt/deadsnakes reachability, actual sm_120 runtime behavior).
- `modules/libraries/spconv/module.yml`, `install.sh` — new `amd64_sm120` combo-key deps (`spconv-cu120` wheel try, soft-fail via `allow_pip_fail: true`) + source-build fallback targeting sm_120 (also **unverified on hardware**).
...
- `stacks/ete-train-5090/stack.yml` (new stack) — `libraries/torch` + `libraries/spconv` + `training/ete-net`, `gpu_arch: sm120`.
- `config/stack.env` — added `RISK_AWARE_PLANNING_SRC` (host path placeholder, edit per-host).
- `scripts/gen_dockerfile_compose.py` — added the `(cpu_arch, gpu_arch)` combo-key dimension (`deps.<arch>_<gpu_arch>`), an opt-in `allow_pip_fail` for wheel-try/source-fallback patterns, `GPU_ARCH` passed into `install.sh` invocations, and amd64 GPU device-reservation in the generated compose file (arm64's `runtime: nvidia` was already there; amd64 had nothing). **Backward compatibility verified**: `python3 scripts/gen_dockerfile_compose.py d435i-voxblox --arch arm64` produces a byte-identical Dockerfile and compose.yml to before this change (diffed directly).

### 2026-07-14, correction (actual host is a 4090, driver 535.183)
- `scripts/gen_dockerfile_compose.py` — extended `base_image` selection to the same
...
- `modules/libraries/torch/install.sh`, `module.yml` — added a `GPU_ARCH` case inside the
...
- `modules/libraries/spconv/module.yml`, `install.sh` — added `deps.amd64_sm89: {pip: [spconv-cu120]}`
...
- `stacks/ete-train-4090/stack.yml` (new stack) — same module list as `ete-train-5090.yml`,
  `gpu_arch: sm89`. `ete-train-5090.yml` itself is untouched (kept for a future actual
  Blackwell host).
- `docs/ETE_TRAIN_5090.md` renamed to `modules/training/ete-net/DEPLOYMENT.md` (this file) and
...
- Added `amd64_sm75` combos for the ML desktop's own RTX 2080 Ti's (Turing): `modules/base/module.yml`
  ...
  driver-535 CUDA<=12.2 ceiling), `modules/libraries/torch/install.sh` (`sm89|sm75` share
  the cu121-wheel branch), `modules/libraries/spconv/module.yml`+`install.sh`
  (`deps.amd64_sm75: {pip: [spconv-cu120]}`, hard-required, same no-source-build-fallback
  policy as sm_89). `stacks/ete-train-2080ti/stack.yml` (new stack, `gpu_arch: sm75`).
- **Real build issues found and fixed** (none of these were caught by static analysis;
  ...
  - This host's dead GPU (PCI `1a:00.0`) breaks the normal `--gpus`/device-reservation
    ...
    `NVIDIA_VISIBLE_DEVICES` path) to `scripts/gen_dockerfile_compose.py`, see the
    "Known host issue" section above.
  - Container writes were root-owned on the bind-mounted `outputs/`/`data/` dirs ->
    added `CONTAINER_USER` to `scripts/gen_dockerfile_compose.py`, see the "File
...
- **Taxonomy rename** (after the parity run completed and results were retrieved, per
  ...
  `stacks/ete-train-{5090,4090,2080ti}.yml` module lists, this doc, `modules/libraries/torch/install.sh`'s
...
- `modules/libraries/torch/wheels/stage_from_archive.sh` (new) — idempotent, md5-verified
  hardlink/copy of the wheel from `TORCH_WHEEL_ARCHIVE_DIR` into this directory.
- `modules/libraries/torch/wheels/README.md` — documented the archive-dir mechanism
...
- **No `install.sh`/`stacks/sim-x86/stack.yml`/generator changes yet** — §1's `build_env`
...
- `modules/libraries/torch/install.sh` — removed the `if [ "${TORCH_VARIANT:-}" =
...
- `modules/libraries/torch/module.yml` — description rewritten: no longer describes the
  wheel as conditional on `build_env`.
- `stacks/sim-x86/stack.yml` — removed the `build_env: {TORCH_VARIANT: src-abi1}` block
  (4 lines + comment); `libraries/torch` module-list comment updated to say why it's
  still first (unchanged reason, worded for the new unconditional default).
- `stacks/ete-train-4090/stack.yml`, `stacks/ete-train-2080ti/stack.yml` — `libraries/torch`
...
- `scripts/gen_dockerfile_compose.py` — removed the `build_env` dimension entirely:
...
- `modules/libraries/torch/wheels/README.md` — rewritten for the unconditional default
...
- `modules/libraries/torch/wheels/stage_from_archive.sh` — header comment updated from
