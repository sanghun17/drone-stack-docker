...
OUTPUT_DIR="${ARUCO_VALIDATION_OUTPUT_DIR:-$ROOT/flight_logs/aruco-landing/controller-validation}"
...
if [ "$BUILD_MAP" = 1 ]; then
  AIRSIM_LANDING_PAD_LAYOUT="$LAYOUT_HOST" AIRSIM_LANDING_PAD_PREFIX="$PAD_PREFIX" \
    "$MODULE/scripts/build_baseline_map.sh" >"$LOG_DIR/map_build.log" 2>&1
fi

"$MODULE/scripts/run_baseline_sim.sh" -RenderOffscreen -graphicsadapter=2 \
...
"$MODULE/scripts/run_trials.sh" "${runner_args[@]}" | tee "$LOG_DIR/trials.log"
