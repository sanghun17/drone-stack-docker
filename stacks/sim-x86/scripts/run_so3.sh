#!/bin/bash
# sim-x86 stack asset: SO(3) control stack (ours_so3_stack.launch = traj_server +
# so3_control_bridge). so3_control_bridge.py talks to AirSim's RPC directly
# (127.0.0.1:41451 on the HOST, reachable here via network_mode:host — no conda
# needed in-container, just the `airsim` pip package from simulation/airsim). No CPU
# pinning (taskset): Jetson-only concern, doesn't apply on the ml desktop.
...
if [ ! -f /.dockerenv ]; then
  ...
  __M="roslaunch local_controller ours_so3_stack.launch"
...
source /work/scripts/lib/ensure_roscore.sh   # master up on $ROS_MASTER_PORT — TCP probe, not a blind sleep 4 (host roscore is expected to already answer here)
