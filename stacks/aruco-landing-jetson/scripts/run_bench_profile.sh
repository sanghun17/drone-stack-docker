...
LAYOUT_HOST="${ARUCO_HARDWARE_LAYOUT:-$ROOT/stacks/aruco-landing-jetson/config/physical_pad.yaml}"
...
OUTPUT_HOST="${ARUCO_BENCH_OUTPUT_DIR:-$ROOT/flight_logs/aruco-landing/hardware-profiles}"
...
sleep 5
docker exec "$CONTAINER" bash -lc \
  "source /opt/ros/noetic/setup.bash; source /work/config/ros_env.sh; exec python3 /work/stacks/aruco-landing-jetson/scripts/profile_pipeline.py --duration '$DURATION' --label '$LABEL' --layout '$LAYOUT_CONTAINER' --dictionary '$DICTIONARY' --pad-size-m '$PAD_SIZE_M' --output-dir '$OUTPUT_CONTAINER' --enable-controller"
